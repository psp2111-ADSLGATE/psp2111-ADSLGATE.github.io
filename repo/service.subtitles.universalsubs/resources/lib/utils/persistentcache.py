# Implementation taken from https://github.com/mariushelf/shelved_cache
...
class PersistentCache(MutableMapping):
    ...
    *kwargs:
    ...
    def __init__(
        ...
        new_cls = type("Wrapped%s" % wrapped_cache_cls.__name__, (DelMixin, wrapped_cache_cls), {})
        if filename:
    ...
    def delete_callback(self, key):
        ...
        try:
            ...
            logger.warning("Key '%s' not in persistent cache." % hkey)
        ...
        if self.filename and self.persistent_dict is None:
            ...
            try:
                ...
                for hk, (k, v) in self.persistent_dict.items():
                    self.wrapped[k] = v
                logger.debug(
                    "Loaded %s cache entries from %s (in cache: %s)." % (
                        len(self.persistent_dict), self.filename, len(self.wrapped))
                )
    ...
    def _destroy_and_reinit_cache(self):
        logger.warning("Failed to open cache database file %s. Overwriting with a new one." % self.filename)
        self.persistent_dict = shelve.open(
            self.filename,
