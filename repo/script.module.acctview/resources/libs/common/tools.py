...
def kill_kodi(msg=None, over=None):
    if over:
        ...
        if not msg:
            msg = '[COLOR {0}]You are about to [B]SHUTDOWN[/B] Kodi. Would you like to continue?[/COLOR]'.format(CONFIG.COLOR2)
        
        choice = dialog.yesno('Power Menu',
                                  msg,
                                  nolabel='[B][COLOR white]NO[/COLOR][/B]',
                                  yeslabel='[B][COLOR white]YES[/COLOR][/B]')
    if choice == 1:
        from resources.libs.common import logging
