...
enable_string, disable_string, specific_string, all_string, scrapers_string, hosters_string = ls(32055), ls(32024), ls(32536), ls(32129), ls(32533), ls(34160)
...
def external_scrapers_choice(params={}):
	...
	hosters_scrapers_string = '%s %s' % (scrapers_string, hosters_string)
	torrent_scrapers_string = '%s %s' % (scrapers_string, torrent_string)
	enable_string_base = '%s %s %s %s' % (enable_string, all_string, scrapers_string, '%s')
	disable_string_base = '%s %s %s %s' % (disable_string, all_string, scrapers_string, '%s')
	enable_disable_string_base = '%s %s %s' % (specific_string, scrapers_string, '%s')
...
def options_menu_choice(params, meta=None):
	...
	if custom_context_menu and not from_extras:
		...
		if menu_type in ('movie', 'tvshow'):
			...
			listing_append((ls(32187), '%s %s' % (ls(32841), ls(32533)), 'playback_choice'))
			if menu_type == 'movie':
	...
	if menu_type in ('movie', 'episode') or menu_type in single_ep_list:
		if menu_type == 'movie' and from_extras: listing_append((ls(32187), '%s %s' % (ls(32841), ls(32533)), 'playback_choice'))
	...
	listing_append(('%s %s' %(ls(32513),ls(32118)), '', 'open_external_scrapers_choice'))
